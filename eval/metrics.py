...
def get_metrics(data, save_dir=None, is_asqa=False, type="base", sample=False):
    ...
    if is_asqa:
        ...
        for d in tqdm(data):
            ...
            ##添加正确输出的id和错误输出的id
            accurate_ids.append(idx) if is_accurate else wrong_ids.append(idx)
    ...
    if is_asqa:
        ...
        id_result = {"accurate_ids": accurate_ids, "wrong_ids": wrong_ids}
    
    if sample:
        with open(f"{save_dir}/metrics_{type}_sample.json", "w") as f:
            f.write(json.dumps(eval_result) + "\n")
        with open(f"{save_dir}/id_{type}_sample.json", "w") as f:
            f.write(json.dumps(id_result) + "\n")   
    else:
        with open(f"{save_dir}/metrics_{type}.json", "w") as f:
            f.write(json.dumps(eval_result) + "\n")
        if not is_asqa:
            with open(f"{save_dir}/id_{type}.json", "w") as f:
                f.write(json.dumps(id_result) + "\n")   
